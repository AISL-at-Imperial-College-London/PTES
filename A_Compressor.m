...
sizes.NumOutputs     = 13;    % outputs of model
sizes.NumInputs      = 9;     % inputs of model
...
Valve_in_gain  = u(8);                                    % opening gain
Valve_out_gain = u(9);                                     % opening gain
% States
...
idle = u(1) == 0;
% idle = 0; 
if idle && (omega_comp <0.5|| m_comp< 0.1) 
    m_in =0;
    m_out = 0;
    m_rec_ss = 0;
else
    m_in     = max(0,Valve_in_gain * Inflow_opening * sqrt(abs(In_pres - p1)) * sign(In_pres - p1));      % Inflow valve
    m_out    = max(0,Valve_out_gain * Outflow_opening * sqrt(abs(p2 - Out_pres)) * sign(p2 - Out_pres)); % Outflow valve
    m_rec_ss = max(0,Valve_rec_gain * Recycle_opening * sqrt(abs(p2 - p1)) * sign(p2 - p1));                   % Recycle valve
end
...
% choose center around which you want it to be most linear
eta_center = eta_mean;   % you can tweak this
k = 5;                   % sharpness; smaller = softer

% map tanh(-inf..inf) -> [0,1]
s = 0.5 * (1 + tanh(k * (eta - eta_center)));

% finally map [0,1] -> [0.82, 0.92]
eta_smooth = 0.82 + s * (0.92 - 0.82);
eta = eta_smooth;
eta = max(eta, 0.15);
eta = min(eta, 0.9);
...
% ******* Outlet temperature  **********
kappa = 1.24;
% kappa = refpropm('K','T',T_in,'P',p1/1000,'CO2');
...
% h1 = Param.splines.H(p1/1000,T_in);
% s1 = refpropm('s','T',T_in,'P',p1/1000,'CO2');
% h2s = refpropm('h','P',p2/1000,'s',s1,'CO2');
% 
% h2 = h1 + (h2s-h1)/eta;
% W = m_comp*(h2-h1)/1000;
% compressor torque
...
sys(3) = m_comp;                    % m_comp kg/s
sys(4) = p_ratio;                    % p_ratio  
sys(5) = omega_comp;
...
sys(13) = idle && (omega_comp <0.5|| m_comp< 0.1);
% sys(13) = m_rec_ss;
...
VolumeT1        = 2; % volume m^3  
VolumeT2        = 1; % volume m^3
...
tauRecycle = 0.1;                            % time constant of the recycle        
...
if idle && (omega_comp <0.5|| m_comp< 0.1)
    ...
    m_in     = 0;      % Inflow valve
    m_out    = 0; % Outflow valve
    m_rec_ss = Valve_rec_gain * Recycle_opening * sqrt(abs(p2 - p1)) * sign(p2 - p1);                   % Recycle valve

else
    sys(3) = 0.15*AdivL*(p_comp-p2);                            % Delta mass in compressor sys(3) = 0.1*AdivL*(p_comp-p2)
